...
class dm_cnn_fc_wa(object):
    def __init__(self, config, sess, train_path, test_path, embedding_path, layer2=False, laptop=False):
        # 模型常量定义
        if laptop == True:
            self.n_epochs = 60
        else:
    ...
    def att_embed(self, batch_embedding):
        ...
        att_filter = tf.Variable(
            tf.truncated_normal(
                [5, self.embedding_size+3*self.ex_emb_size+self.embedding_size, 1],
                mean=0.1,
                stddev=0.1),
            name='att_filter'
        )
    ...
    def att_res_compute(self, batch_embedding):
        ...
        att_res_filter = tf.Variable(
            tf.truncated_normal(
                [1, self.embedding_size + 3*self.ex_emb_size + self.embedding_size, 400],
                stddev=0.1
            ),
            name='att_res_filter'
        )
    ...
    def dmcnn_pooling(self):
        ...
        # 加权之后的embedding: []
        up_res = tf.squeeze(
            self.att_res_compute(
                tf.concat([self.up_connect, self.asp_embed], axis=2)),
            axis=[1]
        )
        down_res = tf.squeeze(
            self.att_res_compute(
                tf.concat([self.down_connect, self.asp_embed], axis=2)),
            axis=[1]
        )
