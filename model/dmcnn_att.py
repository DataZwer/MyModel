...
class dm_cnn_att(object):
    ...
    def att_res_compute(self, batch_embedding):
        ...
        att_res_filter = tf.Variable(tf.truncated_normal([1, self.embedding_size, 500], stddev=0.1), name='att_res_filter')
        att_res_bias = tf.Variable(tf.truncated_normal([500], stddev=0.1), name='att_bias')
    ...
    def dmcnn_pooling(self):
        ...
        # 第二层：微调1
        self.up_compose = self.conv1d_op(self.up_compose)
        self.down_compose = self.conv1d_op(self.down_compose)
        ...
        # 加权之后的embedding
        up_res = tf.squeeze(self.att_res_compute(self.up_compose), axis=[1])  # [b_s, 400]
        down_res = tf.squeeze(self.att_res_compute(self.down_compose), axis=[1])  # [b_s, 1, 400]
    ...
    def softmax_output(self):
        ...
        W = tf.get_variable(
            name='W',
            shape=[(self.num_filters * len(self.filter_sizes) + 500) * 2, self.num_classes],  # 有两边
