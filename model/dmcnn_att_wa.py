...
from keras_data_helper.data_utils_wa import load_all
from bean.Configuration import *
from batch.batches_wa import dataset_iterator
import tensorflow as tf


class dm_cnn_att_wa(object):
    ...
    def input_layer(self):
        self.train_data, self.test_data, self.all_word_index, self.max_len, self.asp_max_len = \
        ...
        self.aspects = tf.placeholder(tf.int32, [None, self.asp_max_len], name='aspects')
    ...
    def embedding_layer(self):
        ...
        # 讲aspect词向量求平均然后拓展开来
        self.asp_embed = tf.nn.embedding_lookup(glove_w2v, self.aspects)  # [b_s, asp_max_len, 300]
        self.asp_embed = tf.reduce_mean(self.asp_embed, axis=1, keepdims=True) / self.asp_max_len  # [b_s, 1, 300]
        self.asp_embed = tf.multiply(
            tf.constant(1, tf.float32, [self.batch_size, self.max_len, self.embedding_size]),
    ...
    def att_embed(self, batch_embedding):
        # 计算注意力
        att_filter = tf.Variable(tf.truncated_normal([5, self.embedding_size, 1], stddev=0.1), name='att_filter')
    ...
    def att_res_compute(self, batch_embedding):
        ...
        att_res_filter = tf.Variable(tf.truncated_normal([1, self.embedding_size, 400], stddev=0.1), name='att_res_filter')
    ...
    def conv1d_op(self, embedding):
        ...
        b = tf.Variable(tf.constant(0.1, shape=[self.embedding_size]), name='conv1d_b')
        # conv layer
        # [b_s, max_len, 300]
    ...
    def gru_embedding(self, batch_embedding):
    ...
    def dmcnn_pooling(self):
        ...
        with tf.variable_scope('up'):
            self.up_compose = self.bi_gru_embedding(self.up_compose)
        with tf.variable_scope('down'):
            self.down_compose = self.bi_gru_embedding(self.down_compose)

        # 加权之后的embedding
        up_res = tf.squeeze(
            self.att_res_compute(tf.add(self.up_compose, self.asp_embed)),
            axis=[1]
        )
        down_res = tf.squeeze(
            self.att_res_compute(tf.add(self.up_compose, self.asp_embed)),
        ...
        # cov2d
        self.up_compose = tf.expand_dims(self.conv1d_op(self.up_compose), -1)
        self.down_compose = tf.expand_dims(self.conv1d_op(self.down_compose), -1)
        ...
        up_concat = tf.concat([up_res, up_conv_pool], axis=1)  # [b_s, 700]
        down_concat = tf.concat([down_res, down_conv_pool], axis=1)  # [b_s, 700]
        # [b_s, 1400]
    ...
    def train(self):
        ...
        for epoch in range(self.n_epochs):
            ...
            for iter in range(iterations):
                ...
                word_down_batch, position_down_batch, poses_down_batch, sts_down_batch, asp_batch, \
